...
- チャット履歴欄のうちメッセージや進捗カードがない空白領域でも、アプリウィンドウをドラッグ移動できるように変更。

## ver 0.3.14

- 入力ボックス下段のマイクボタンと設定ボタンの間に、送信ボタンと新規チャット作成ボタンを再追加。
- Codex 実行中は送信ボタンを停止ボタンへ切り替え、実行中の待機処理と承認待ちをキャンセルできるように変更。

## ver 0.3.13

- チャット入力ボックスを二段構成に変更し、上段をテキスト入力、下段を既存ボタン・モデル選択・音声・設定アイコンの操作列に整理。
- 設定アイコンを入力ボックス内へ移動し、狭幅時でもテキスト入力欄の横幅を確保しやすいレイアウトへ変更。

## ver 0.3.12

- ウィンドウ幅が狭いときも設定ボタンがチャット入力欄の次段に回らないよう、入力行のレイアウトを1行維持に変更。
- 狭幅時は Codex モデル/推論表示を `5.6 Sol/Mid` 形式の短縮表示にし、未入力時 placeholder は `Welc...` 表示へ切り替えるように変更。

## ver 0.3.11

- アプリの表示 / 非表示ショートカットを `Ctrl + Alt + M` から `Alt + Shift + M` に変更。
- 設定画面のショートカット表示も `Alt + Shift + M` に更新。

## ver 0.3.10

- 保存済み設定に存在しない Codex モデルが残っている場合、起動時にモデルリスト内の既定モデルへ補正するように修正。
- 入力欄のモデル表示と送信時のモデル値もモデルリスト内の値へ正規化するように変更。
